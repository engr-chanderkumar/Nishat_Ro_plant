...
const EditSaleModal: React.FC<EditSaleModalProps> = ({ isOpen, onClose, onUpdateSale, sale, customers, salesmen, inventory }) => {
    ...
    useEffect(() => {
        if (sale) {
            ...
            setSalesmanId(sale.salesmanId ? sale.salesmanId.toString() : '');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const updatedSale: Sale = {
            ...
            inventoryItemId: selectedInventoryItemId ? Number(selectedInventoryItemId) : null,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md max-h-full overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold text-brand-text-primary">Edit {isPaymentOnly ? 'Payment' : 'Sale'} for {customerName}</h2>
                    ...
                    <div>
                     ...
                     <div>
                        <label htmlFor="edit-item-select" className="block text-sm font-medium text-brand-text-secondary">Item</label>
                         <select id="edit-item-select" value={selectedInventoryItemId} onChange={e => setSelectedInventoryItemId(e.target.value)} required disabled={isPaymentOnly} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm disabled:bg-gray-100">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="edit_quantity" value={quantity} onChange={e => setQuantity(Number(e.target.value))} required min="0" disabled={isPaymentOnly} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm disabled:bg-gray-100" />
                        </div>
                        <div>
                           <label htmlFor="edit_unitPrice" className="block text-sm font-medium text-brand-text-secondary">Unit Price (PKR)</label>
                           <input type="number" id="edit_unitPrice" value={unitPrice} onChange={e => setUnitPrice(e.target.value === '' ? '' : Number(e.target.value))} required min="0" disabled={isPaymentOnly} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm disabled:bg-gray-100" />
                        </div>
                    </div>
                    <div>
                        <label htmlFor="edit_empties" className="block text-sm font-medium text-brand-text-secondary">Empties Collected</label>
                        <input type="number" id="edit_empties" value={emptiesCollected} onChange={e => setEmptiesCollected(Number(e.target.value))} required min="0" disabled={isPaymentOnly} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm disabled:bg-gray-100" />
                    </div>
                     <div>
                     ...
                     <div>
                        ...
                        <select id="edit-salesman-assign" value={salesmanId} onChange={e => setSalesmanId(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm">
